...
makeGoldStandardNets = function(P)
{
  ...
  wsHigh = sample_smallworld(dim=1,size=P, nei=5, p=0.5)
  wsHighDensity = length(E(wsHigh))/(P*PminusOne/2)
  wsLow = sample_smallworld(dim=1,size=P, nei=2, p=0.5)
  ...
  hsHigh = sample_pa(n=P, power=2, m=5, directed=F)
  hsHighDensity = length(E(hsHigh))/(P*PminusOne/2)
  hsLow = sample_pa(n=P, power=2, m=2, directed=F)
